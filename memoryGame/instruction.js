...
selectDiffEasy.addEventListener("click", function(event) {
  if (window.confirm("Czy chcesz rozpocząć nową grę na poziomie łatwym?")) {
    button.classList.remove("menuButtonHidden");
    button.setAttribute("class", "menuButtonShown");
    resetGame();
    pauseGame();
...
selectDiffHard.addEventListener("click", function(event) {
  if (window.confirm("Czy chcesz rozpocząć nową grę na poziomie trudnym?")) {
    console.log("toggling class");
    button.classList.remove("menuButtonHidden");
    button.setAttribute("class", "menuButtonShown");
    resetGame();
...
function checkDifficulty() {
  return document.querySelectorAll(".gridContainer .card").length;
} // 16
function checkDifficultyHard() {
  console.log(document.querySelectorAll(".gridContainerHard .card").length);
} // 36
...
highScoresListHard.innerHTML = highScoresHard
  .map(finalscore => {
    return `<li class="high-score">${finalscore.login}-${finalscore.score +
      5}</li>`;
  })
  .join("");
